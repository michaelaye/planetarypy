...
class TestResolveByPattern:
    def test_unknown_type_returns_none(self, monkeypatch):
        monkeypatch.setattr(
            "planetarypy.catalog._pattern_resolver._get_url_pattern",
            lambda m, i, pk: None,
        )
    ...
    def test_with_mocked_pattern(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "planetarypy.catalog._pattern_resolver.get_product_type_file_info",
            lambda m, i, pk: ("D", ".IMG", ".LBL"),
        )
    ...
    def test_with_urn_pattern(self, monkeypatch):
        ...
        assert result is not None
        assert "my_obs_20200101.xml" in result.files
        assert "my_obs_20200101.dat" in result.files
...
class TestHasPattern:
    def test_unknown_returns_false(self, monkeypatch):
        monkeypatch.setattr(
            "planetarypy.catalog._pattern_resolver._get_url_pattern",
            lambda m, i, pk: None,
        )
