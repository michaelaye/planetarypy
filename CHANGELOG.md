...
- **`plp spicer` prints a Rich table** instead of hand-aligned text, grouped into shape, Sun and light time, and the surface point when `--lon/--lat` are given. Angles now carry a degree sign. Hints (a missing mission metakernel, say) print below the table, where long lines stay readable.
