...
download that ``planetarypy.constants`` performs (a deliberate trade — see the
module's lazy cache).
...
def body_crs(body, system: str = "ocentric", year: int = 2015) -> CRS:
    """Return a body's geographic CRS from the IAU authority.
    ...
    year : int
        IAU constants edition (e.g. 2015, 2009).
    ...
    try:
        return CRS.from_authority(f"IAU_{year}", code)
    except CRSError:
        raise ValueError(
            f"No IAU_{year} {system!r} CRS for body {body!r} (code {code}). "
            "The body may only define an 'ocentric' system, or be absent "
            f"from the IAU_{year} edition."
        ) from None


def local_crs(
    lon: float, lat: float, body, *, system: str = "ocentric", year: int = 2015
) -> CRS:
    ...
    geodetic = body_crs(body, system, year).geodetic_crs
    return ProjectedCRS(
...
def get_crs(body, system: str = "default", year: int = 2015) -> CRS:
    ...
    if system == "default":
        system = "ocentric"
    return body_crs(body, system, year)
